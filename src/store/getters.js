...
const getters = {
  ...
  isLoggedIn: (state) =>
    state.bearerToken && Object.keys(state.profile).length !== 0, //
  isFreeloader: (state) => (!!state.profile?.is_freeloader),
  mustPhoneVerified: (state) =>
    state.user?.phone_verified_at === null && state.isPhoneModify,
  countryList: (state) =>
    state.countries.map(function (country) {
      return {
        id: country.id,
        code: country.name
      }
  ...
  instagram: (state) => {
    if (
      state.profile.instagram !== null &&
      state.profile.instagram !== undefined
    ) {
  ...
  facebook: (state) => {
    if (
      state.profile.facebook !== null &&
      state.profile.facebook !== undefined
    ) {
  ...
  youtube: (state) => {
    if (
      state.profile?.youtube !== null &&
      state.profile?.youtube !== undefined
    ) {
  ...
  twitter: (state) => {
    if (
      state.profile?.twitter !== null &&
      state.profile?.twitter !== undefined
    ) {
