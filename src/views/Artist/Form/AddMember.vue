    ...
    <form @submit.prevent="submit" class="modal-add-member">
      <div class="form-group text-center upload-img">
        <label class="label-img">
          <span class="material-symbols-outlined camera-inner" v-if="!avatar"
            >&#xe412;</span
          >
          <img
            :src="avatar"
      ...
      <div v-for="err in errors?.member_avatar" :key="err" class="text-danger">
        {{ err }}
      </div>
      ...
      <div class="form-group">
        ...
        />
        <div
          v-for="err in error?.member_name"
          :key="err"
      ...
      <div class="form-group form-group-last">
        ...
        <select v-model="form.role" class="form-select" required>
          ...
          <option
            v-for="member_role in roles"
            :key="member_role.id"
            :value="member_role.value"
          >
            {{ member_role.label }}
          </option>
        </select>
...
export default {
          ...
          !(
            this.roles.filter(
              (el) =>
                ...
                this.members[val].role?.toLocaleLowerCase()
            ).length > 0
          )
        ) {
        ...
        this.form = {
          ...
          role:
            this.roles.filter(
              ...
              ? this.members[val]?.role.toLowerCase() || ""
              : "others",
        };
      ...
      if (
        !(
          this.roles.filter(
            (el) =>
              el.value.toLocaleLowerCase() ===
              this.member.role.toLocaleLowerCase()
          ).length > 0
        )
      ) {
  ...
  methods: {
    ...mapActions(["addMember", "fetchMember", "updateMember"]),
    changeImage(event) {
