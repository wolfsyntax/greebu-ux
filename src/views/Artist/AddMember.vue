          ...
          <div class="col">
            ...
            <div class="form-group text-center upload-img">
              <label class="label-img">
                <span class="material-symbols-outlined camera-inner"
                  >&#xe412;</span
                >
                <div class="camera">
                  <input
                    type="file"
                    @input="form.member_avatar = $event.target.files[0]"
                    class="member-avatar"
                    accept="image/png, image/webp, image/svg, image/jpeg"
                  />
                  <span v-if="errors?.member_avatar" class="text-danger">{{
                    errors.member_avatar.shift()
                  }}</span>
                </div>
        ...
        <div class="row py-2">
          <div class="col">
            <div class="form-group">
              ...
              />
              <span
                v-if="errors?.member_name"
                class="member-name text-danger"
                >{{ errors.member_name.shift() }}</span
              >
            </div>
          </div>
        </div>
        <div class="row py-2">
          <div class="col">
            <div class="form-group">
              ...
              <span v-if="errors?.role" class="role-error text-danger">{{
                errors.role.shift()
              }}</span>
            </div>
          </div>
