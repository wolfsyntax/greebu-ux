        ...
        <div class="container">
          ...
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
              <li class="nav-item">
                <router-link to="/" v-if="['', 'customers'].includes(userRole)"
                  >Home</router-link
                >
              </li>
              <li class="nav-item">
                <router-link
                  to="/library"
                  v-if="['artists', 'organizer'].includes(userRole)"
                  >Library</router-link
                >
              </li>
            ...
            <div class="float-end nav-button" v-if="!isLoggedIn">
              <router-link to="/login" class="btn btn-primary log-in"
                >Log In</router-link
              >
              <router-link to="/register" class="btn btn-secondary sign-up"
                >Sign Up</router-link
              >
            </div>
            ...
            <div class="float-end nav-button" v-else>
              ...
              <div class="dropdown dropstart">
                ...
                <ul
                  class="dropdown-menu dropdown-menu-end dropdown-menu-lg-start p-0"
                  id="dropdownNotif"
                  data-bs-autoClose="false"
                >
                  <li class="title-wrap">
                    <span class="title m-0">Notifications</span>
                  </li>
                      ...
                      <div class="notif-msg-wrap">
                        <div class="msg-wrap">
                          <p class="m-0 sender-msg">
                            ...
                            Your free trial ends in 3 days. To avoid any
                            interruption in your access, Subscribe now!
                  ...
                  <li
                    v-for="(content, index) in notifications"
                    :key="index"
                    class="sender-notif-wrap"
                  >
                    <div class="d-flex align-items-start notif-wrap">
                      <div class="img-wrap">
                        <img
                          :src="content?.data?.sender_avatar"
                          class="border-0 sender-img"
                          alt="Sender Avatar"
                        />
                      </div>
                      <div class="notif-msg-wrap">
                        <div class="msg-wrap">
                          <p
                            class="m-0 sender-msg"
                            v-if="
                              content?.data?.notification_type ===
                              'artist-proposal'
                            "
                          >
                            <span class="m-0 sender-name">{{
                              content?.data?.sender_name
                            }}</span
                            >&nbsp;{{
                              `${content?.data?.header} ${content?.data?.misc?.event_name}`
                            }}!
                          </p>
                        </div>
                        <div class="date-time-wrap">
                          ...
                          >
                            <small>{{
                              $filters.diffForHumans(
                                $moment(content?.data?.time).format(
                                  "YYYY-MM-DD hh:mm:ss a"
                                )
                              )
                            }}</small>
                            <small class="float-end">{{
                              $moment(content?.data?.time).format("MM/DD/YYYY")
                            }}</small>
                          </p>
                        </div>
                      </div>
                  ...
                  <div
                    class="d-flex align-items-center justify-content-between mb-0 action-wrap"
                  >
                    <button
                      class="btn float-end mb-0 all-read"
                      @click="markAsRead"
                    >
                      Mark all as read
                    </button>
                    <router-link
                      to="/account/setting"
                      class="btn all-notif"
                      @click="$store.commit('setSettingMenu', 'Notifications')"
                ...
                >
                  <li>
                    <div class="d-flex align-items-start artist-info">
                      ...
                      <div class="artist-name">
                        <p class="two-lines name">
                          {{ userInfo.business_name }}
                        </p>
                        <!-- <p class="email">{{ userInfo.business_email }}</p> -->
                        <p class="email" style="text-transform: capitalize">
                          {{ userInfo.role }}
                        </p>
                        <router-link
                          to="/account/profile"
                  ...
                  <li>
                    ...
                    <router-link
                      to="/account/setting"
                      class="dropdown-item"
                      @click="$store.commit('setSettingMenu', 'My Account')"
                  ...
                  <li v-if="['artists'].includes(userRole)">
                    ...
                    <router-link
                      to="/account/setting"
                      @click="
                        $store.commit('setSettingMenu', 'Customized Songs')
                      "
                      class="dropdown-item"
                      >Customized Songs</router-link
                    >
                  </li>
                  ...
                  <li v-if="['customers'].includes(userRole)">
                    ...
                    <router-link
                      to="/account/setting"
                      @click="$store.commit('setSettingMenu', 'My Songs')"
                      class="dropdown-item"
                  ...
                  <li v-if="['customers', 'artists'].includes(userRole)">
                    ...
                    <router-link
                      to="/account/setting"
                      @click="$store.commit('setSettingMenu', 'My Bookings')"
                      class="dropdown-item"
                  ...
                  <li v-if="['artists'].includes(userRole)">
                    ...
                    <router-link
                      to="/account/setting"
                      @click="
                        $store.commit('setSettingMenu', 'Manage Subscriptions')
                      "
                      class="dropdown-item"
                  ...
                  <li v-if="['customers', 'organizer'].includes(userRole)">
                    ...
                    <router-link
                      to="/account/setting"
                      @click="$store.commit('setSettingMenu', 'My Proposal')"
                      class="dropdown-item"
                      >My Proposals</router-link
                    >
                  </li>
              ...
              <div class="card" style="width: 31.5rem">
                <div class="card-body">
                  ...
                  <p class="card-text">
                    ...
                    bibendum. Metus et lectus id viverra fringilla magna morbi.
                  </p>
                </div>
              </div>
            </div>
