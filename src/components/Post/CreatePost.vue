<template>
  <div class="modal fade " id="artistPost"  data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content" v-if="selectedItem === null || selectedItem === 'photo' || selectedItem === 'music'">
        <div class="modal-header">
          ...
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" ref="closePostModal" ></button>
        </div>

        <div class="modal-body">
          <div class="d-flex align-items-center artist-image">
            <img :src="myAvatar" alt="profile">
            <h3 class="band-name">{{ profile?.business_name || 'Idle Pitch' }}</h3>
          </div>

          <form @submit.prevent="submitForm">
            <div class="form-group">
              <textarea class="form-control" v-model="formData.message" placeholder="Write something..." ref="postContent"></textarea>
            </div>

            <!-- PHOTO/VIDEO SELECTED -->
            <div v-if="selectedItem === 'photo'" class="upload-file-wrapper" @dragover="handleDragOverFile"
              @dragleave="handleDragLeaveFile" @drop="handleDropFile" :class="{ 'drag-over': isDragOverFiles }">
            ...
            <!-- MUSIC SELECTED -->
            <div v-if="selectedItem === 'music'" class="upload-file-wrapper" @dragover="handleDragOverMusic"
              @dragleave="handleDragLeaveMusic" @drop="handleDropMusic" :class="{ 'drag-over': isDragOverMusic }">
              <div class="text-center upload-file-content">
                <svg xmlns="http://www.w3.org/2000/svg" width="45" height="59" viewBox="0 0 45 59" fill="none">
                  ...
                  <defs>
                    <clipPath id="clip0_3478_73591">
                      <rect width="45" height="58" fill="white" transform="translate(0 0.5)" />
                    </clipPath>
                  </defs>
                </svg>
                ...
                <div class="upload-wrapper">
                  ...
                  <input type="file" class="upload-music" id="files" style="display:none;" ref="musicInput"
                    @change="handleMusicUpload" accept="audio/*" />
                </div>
              </div>
            </div>
            ...
            <div class="d-flex align-items-center manage-to-post">
              <div class="d-flex align-items-center feeling">
                <button type="button" class="btn d-flex align-items-center" @click="toggleContent('feeling')">
                  <span
                    :class="{ 'material-symbols-rounded post-icon': true, 'selected': selectedItem === 'feeling' }">&#xe813;</span>
              ...
              <div class="d-flex align-items-center photo-video">
                <button type="button" class="btn d-flex align-items-center" @click="toggleContent('photo')">
                  <span
                    :class="{ 'material-symbols-rounded post-icon': true, 'selected': selectedItem === 'photo' }">&#xe3f4;</span>
              ...
              <div class="d-flex align-items-center music">
                <button type="button" class="btn d-flex align-items-center" @click="toggleContent('music')">
                  <span
                    :class="{ 'material-symbols-rounded post-icon': true, 'selected': selectedItem === 'music' }">&#xe030;</span>
      ...
      <div class="modal-content" v-else-if="selectedItem === 'feeling'">
        <div class="modal-header">
          <h2 class="modal-title" id="artistPostLabel">
            <span class="material-symbols-rounded back-to-post"
              @click="toggleContent('feeling')">arrow_back</span>Feelings
          </h2>
...
export default {
  ...
  computed: {
    ...
    myAvatar()
    {
      return this.profile?.avatar || this.account?.avatar || 'https://res.cloudinary.com/daorvtlls/image/upload/v1686465790/cld-sample.jpg';
    },
    iconList()
    {
  ...
  mounted()
  {
    const myModal = document.getElementById('artistPost');
  ...
  methods: {
    ...
    async submitForm()
    {
      ...
      }).then(response =>
      {
        ...
        if (status === 200) {
          const { post } = data;
          this.$emit('submitData', post);
          this.$refs.closePostModal.click();
    ...
    toggleContent(item)
    {
      if (this.selectedItem === item) {
        ...
        this.selectFile = false; // No attachment
      } else {
