<template>
            ...
            <li class="list-group-item" v-for="(mem, index) in members" :key="mem.id">
              ...
              <div class="options"> 
                <div class="d-flex align-items-center text-end">
                  <button type="button" class="edit-band-member-wrapper" @click="showMemberModal" data-bs-toggle="modal" data-bs-target="#exampleModalUpdate">
    ...
    <UpdateBandMember :members="members"  @updateBandMember="submitUpdate" />
  </template>
  
  <script>
  import { mapGetters, mapState, mapActions } from "vuex";
  import MemberForm from '/src/views/Artist/Form/AddMember.vue';
  import UpdateBandMember from "/src/components/Artist/UpdateBandMember.vue";
  export default {
    components: {
    ...
    showMemberOptions(memberId){
      ...
      submitUpdate(updatedMember){

        const index = this.members.findIndex((mem) => mem.id === updatedMember.id);
      if (index !== -1) {
