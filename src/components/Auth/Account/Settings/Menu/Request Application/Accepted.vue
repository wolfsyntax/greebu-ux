<template>
  <div>
    <div class="customized-songs-wrapper" v-for="( accepted, index) in acceptedApplication" :key="index">

      <div class="event-description">
        <h5 class="event">{{  accepted.event }} <span class="in-progress">Ongoing</span></h5>

        <div class="mb-0 d-flex align-items-center requested-wrapper">
          <img :src= accepted.profile_image class="requested-by-image" alt="Requested by image"> 
          <a href="#" class="requested-by">Requested by 
          <span class="name">{{  accepted.name }}</span></a> 
          <span class="material-symbols-sharp dot-icon">&#xe061;</span>
          <span class="time">{{  accepted.time }} hours ago</span>
        </div>
