<template>
  <div>
    <form class="required-fields" @submit.prevent="submit">
      ...
      <drag-drop @dragCover="setCover" v-else />
      <div
        v-for="err in error?.cover_photo"
        :key="err"
...
export default {
  ...
  data: () => ({
    ...
    isLoading: false,
    isComplete: false,
    errorTime: "Event Start date should be 5 days ahead.",
    // form: {
    //   cover_photo: '',
    //   event_type: '',
    //   event_name: '',
    ...
    validInput() {
      if (this.isComplete) {
        if (this.$moment(this.eventEnd).isAfter(this.eventStart)) {
          this.errorTime = "Event Start date should be 5 days ahead.";
          return true;
        } else {
